...
export default function Overlay() {
  ...
  useFrame(() => {
    setOp1(1 - scroll.range(0, 1 / 14));
    setOp2(scroll.curve(1 / 13, 1 / 7));
    setOp3(scroll.curve(0.33, 1 / 7));
    setOp4(scroll.curve(4 / 7, 1 / 7));
  });
  return (
    <Scroll html>
      <div
      ...
      >
        <h1 className="visible md:font-sans text-4xl md:text-7xl lg:text-9xl text-white max-sm:hidden">
          Hi I'm <span className="font-bold bg-red-600 p-1">Chaand</span>
        </h1>
      ...
      <div className="h-screen grid grid-cols-2" style={{ opacity: op3 }}>
        <h1 className="justify-self-center text-3xl md:text-4xl lg:text-7xl text-white">
          I ride <span className="font-bold bg-slate-400 p-1">Motorcycles</span>
        </h1>
      </div>
      <div className="h-screen grid grid-cols-2" style={{ opacity: op4 }}>
        <h1 className="justify-self-center text-3xl md:text-4xl lg:text-7xl text-white">
          I love <span className="font-bold bg-red-400 p-1">Basketball</span>
        </h1>
      </div>
    </Scroll>
  );
